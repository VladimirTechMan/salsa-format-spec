...
When using the "plain-text" or "plain-text-multipart" format options, the SALSA file creator MUST ensure that all the string values representing packet bodies are valid in terms of the UTF-8 encoding scheme. Where the SALSA file creator is a part of application that already does this check or otherwise guarantees the usage of UTF-8 (for example, when the application is the actual source of the packets being logged and is known to always use UTF-8 for them), the SALSA file creator MAY rely on those known checks as a matter of validating the strings representing packet bodies.
