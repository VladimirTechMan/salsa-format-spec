...
In the current version of SALSA, the "format" string MUST have one of three possible values: "plain-text", "plain-text-multipart" and "base64". The "format" string is optional: When it is not provided for a "packet" object, the "plain-text" value MUST be assumed by default.
...
* When the "format" parameter value is "plain-text", the "body" value in the "packet" object MUST be a string properly encoded in UTF-8. The contents of the string MUST be byte-equal to the body of the original packet that it represents in the archive. (Thus, this encoding only can be applied when the original packet is properly encoded using UTF-8).
* When the "format" parameter value is "plain-text-multipart", the "body" value in the "packet" object MUST be an array of strings, each of which is properly encoded in UTF-8. The concatenation of those strings, in the order they have inside the array, must be a string byte-equal to the body of the original packet that it represents in the archive. (Thus, this encoding only can be applied when the original packet is properly encoded using UTF-8).
* When the "format" parameter value is "base64", the "body" value in the "packet" object MUST be a string properly encoded in UTF-8. The string MUST represent a byte sequence correctly encoded using base64, as described in RFC 4648. When decoded back to a byte sequence, in accordance with RFC 4648, the resulting byte sequence MUST be byte-equal to the body of the original packet that it represents in the archive. This encoding can be applied to represent any textual or binary packet bodies in a SALSA file.

The SALSA file creator SHOULD use either "plain-text" or "plain-text-multipart" formats whenever the body of the original packet is already known to have a valid UTF-8 encoding or there is a way to verify that it has a proper UTF-8 encoding. When not using either "plain-text" or "plain-text-multipart" format, the SALSA file creator MUST use the "base64" encoding to represent the packet body.

It is RECOMMENDED to use the "plain-text-multipart" format when it is desireable that the targer IT personnel can directly inspect (read) the resulting SALSA file as a plain text file. In this case, it is also RECOMMENDED to split the original packet body into a sequence of strings so that each string matches a separate line in the original packet body and is finished by the end-of-line character seqence, in accordance with the specifications of the signaling protocol of the packet being archived.

Whenever a more compact representation of a file in the SALSA format is desireable, it is RECOMMENDED to use the "plain-text" format. The final choice between one of the two available plain text formatting options, for UTF-8 encoded packet bodies, remains with the SALSA file creator. (For example, the choice may be controlled by an external profile settings or directly by the operator person using an application that includes the SALSA file creator.)
