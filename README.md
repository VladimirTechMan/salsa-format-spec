...
This object represents the root of the exported data. This object MUST be present and its name MUST be "salsa". The object contains the following name/value pairs:
 ...
 "startedDateTime" | string | Optional. The timestamp for the moment when the application became ready to capture the packets represented in the given SALSA-formatted file.
...
A SALSA file creator application SHOULD provide the "startedDateTime" value. For a specific SALSA-formatted file, the "startedDateTime" string value MUST represent the moment when the application that captured the packets archived in this file became fully ready to capture them. In the simplest case, that moment is when the first packet was actually captured by the application. But it may also be earlier on the timeline, for example, if the application was waiting for some time before it actually received and captured the first signalling packet.

When the "startedDateTime" value is provided in the file, all the individual "time" values of "packet" objects, in the "packets" array, MUST be calculated relative to that "startedDateTime" value. When the "startedDateTime" value cannot be provided in the file, the SALSA file creator application MAY still be able to provide the "time" values for "packet" objects relative to the moment when the capturing was actually started (for example, when converting to SALSA format from an existing capture file in a format that provides that relative timing information in it, but not the absolute start time of capturing packets). If the SALSA file creator application has no information to determine the time stamps of individual packets relative to the moment when the capturing activity was started, then it MUST assume it to be equal to the moment when the first packet was actually captured (and, thus, the relative time stamp value for that first packet will be zero in the "packets" array).

The "startedDateTime" string value, when provided, MUST be formatted in accordance with the ISO 8601 format for combined date and time, and it MUST include seconds as its smallest time unit provided, with, at least, thousandth fractions provided in the seconds values, to support the millisecond precision. The SALSA file creator application MAY add the time zone information, in accordance to ISO 8601 specifications, when it is directly available or can be easily determined by the application. Otherwise, the "startedDateTime" string value specifies the local time for the system where the packets were captured. Thus the format of "startedDateTime" string value MUST be "YYYY-MM-DDThh:mm:ss.sss" when representing a local time (for example, "2013-10-22T17:18:30.457") and it MUST be "YYYY-MM-DDThh:mm:ss.sssTZD" when representing a global time, relative to UTC (for example, "2013-10-22T17:18:30.457+03:00"). In the latter case, for a file comliant with this version of SALSA format spec, the time zone MUST be specified in the form "+hh:mm" or "-hh:mm", depending on the local time offset relative to UTC. (Other possible formats declared in ISO 8601 for specifying time zones are not valid in SALSA. That is for the sake of easier parsing and formatting.)

The usage of ISO 8601-compliant format, as opposed to a numeric value equal the absolute clock time on the system, is intentional in SALSA as it allows for more compatibilty between different systems when doing some calendar calculations, even basic ones. (The absolute time values on a system are normally relative to some predefined epoch and different systems may have different epoch "starting points". Thus, a more portable approach is beneficial.) Also, the string format specified above is easier to understand for a person reading a SALSA-formatted file.
